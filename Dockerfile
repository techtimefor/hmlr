...
# Enable source repositories for Ubuntu 24.04 (Noble)
...
WORKDIR /build/Katie
# Use the direct path for build-dep
RUN apt-get build-dep -y ./package/debian && \
...
# The "Mop-up" fix for the dependencies you saw earlier
...
# Use the direct path to the packaging folder
RUN apt-get build-dep -y ./packaging/debian/katanalibs && \
    ln -sv packaging/debian/katanalibs debian && \
    dpkg-buildpackage -uc -us -b
